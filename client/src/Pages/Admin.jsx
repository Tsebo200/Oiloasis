...
function Admin() {
  ...
  useEffect(() => {
    ...
    .then((res)=>{
      ...
      let renderProducts = productData.map((item) => <AdminProductCardInfoCard key={item._id} quantity={item.quantity} productName={item.productName} size={item.size} color={item.color} productPrice={item.productPrice} stock={item.stock} description={item.description} disclaimer={item.disclaimer}/>)
